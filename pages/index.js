          ...
          <header className='px-2 w-full py-2 flex justify-between flex-wrap bg-gray-50 border-b-2 border-b-gray-100'>
            <div className='w-fit flex py-1'>
              <ButtonBorder
                ...
                otherStyles={
                  postAnswerToggle === 'POSTS' ? 'bg-emerald-600' : ''
                }
...
export const getServerSideProps = async (context) => {
  try {
    ...
    contributesToShow = await Promise.all(
      contributesToShow.map(async (post) => {
    ...
    console.log('postsToShow', contributesToShow)
